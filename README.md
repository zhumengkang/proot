...
1. Fork 或克隆本仓库到您的 GitHub 账户
2. 登录 EdgeOne Pages 控制台，创建新项目
3. 连接您的 GitHub 仓库
4. 使用以下设置：
   - 构建命令：留空（无需构建）
   - 输出目录：留空（默认为根目录）
5. **⚠️ 重要：在项目设置中添加环境变量 `PASSWORD`**
6. **可选：在项目设置中添加环境变量 `ADMINPASSWORD`**
7. 点击"部署"

> 注意：EdgeOne Pages 使用兼容的中间件处理环境变量，支持多种环境变量访问方式。
